...
### Step 8: Build the Microsoft Power Automate Flow

Power Automate now acts as the orchestration layer between Azure Functions and Microsoft Teams.
...
4. Configure the request schema using the fields sent by `IdentityAuditTimer`.
...
7. Add that URL to the Azure Function App as:
...
#### 8.2 Post the Adaptive Card to Microsoft Teams
...
Use the following Adaptive Card:
...
The important difference from the old implementation is that **Teams does not directly call the Azure Function from an `Action.Http` card**. Instead:
...
This keeps the interactive Teams action inside the Power Automate workflow.

---

#### 8.3 Add the HTTP remediation action

After the **Post adaptive card and wait for a response** action, add:
...
```json
{
  "user_id": "@{outputs('Post_adaptive_card_and_wait_for_a_response')?['body/data/user_id']}",
  "action": "@{outputs('Post_adaptive_card_and_wait_for_a_response')?['body/data/action']}"
}
```

> If Power Automate gives the action a different internal name, select the corresponding dynamic content/output from the **Post adaptive card and wait for a response** action instead of manually typing the expression.

---

#### 8.4 Post the confirmation to Teams

Add a final **Post message in a chat or channel** action.

Example confirmation:

```text
✅ Remediation Success: Account @{outputs('Post_adaptive_card_and_wait_for_a_response')?['body/data/user_id']} disabled.
```

A production flow should also include a failure branch that reports remediation errors rather than displaying a success message when the HTTP action fails.
